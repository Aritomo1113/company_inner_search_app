...
with st.chat_message("assistant"):
    try:
        ...
        # 🔍 参照元（PDFの場合はページ番号付きで表示）
        if "source_info" in llm_response and llm_response["source_info"]:
            st.markdown("### 🔍 参照元ドキュメント")
            for src in llm_response["source_info"]:
