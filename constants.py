...
# ===============================
# 読み込み対象ファイルの拡張子設定（統一定義）
# ===============================
# ※ ここで一度だけ定義します（複数定義しないことが重要）
# - .txt は TextLoader を使って UTF-8 で読み込むように設定済み
SUPPORTED_EXTENSIONS = {
    ".pdf": PyMuPDFLoader,                                    # PDF
    ".docx": Docx2txtLoader,                                  # Word
    ".csv": lambda path: CSVLoader(path, encoding="utf-8"),   # CSV（エンコーディング明示）
    ".txt": lambda path: TextLoader(path, encoding="utf-8")   # TXT（日本語対応）
}
...
# ==========================================
# RAG関連の定数（マジックナンバーの変数化）
# - ベクターストアから取得する関連ドキュメント数
# - ドキュメントのチャンク分割に使用するチャンクサイズとオーバーラップ
# ==========================================
RAG_TOP_K = 5
CHUNK_SIZE = 500
CHUNK_OVERLAP = 50
